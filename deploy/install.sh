...
for required in bot.py discord_app.py requirements.txt .env; do
...
install -o root -g ptvbot -m 0640 "$SOURCE_DIR/discord_app.py" "$TARGET_DIR/discord_app.py"
...
if [[ -f "$SOURCE_DIR/discord_app.db" ]]; then
  install -o ptvbot -g ptvbot -m 0640 \
    "$SOURCE_DIR/discord_app.db" \
    /var/lib/ptv-disruptions/discord_app.db
fi
